d3 = function() {
  ...
  d3.layout.hierarchy = function() {
    var sort = d3_layout_hierarchySort, children = d3_layout_hierarchyChildren, deptreenodelist=d3_layout_hierarchyDepTreeNodelist,value = d3_layout_hierarchyValue;
    
    function isexistelem(node,list){
        for(var i=0;i<list.length;i++){
            if(list[i].actionid==node.actionid)
                return true;
        }
    }

    function previousrecurse(node,depth,nodes){
        var deplist=deptreenodelist.call(hierarchy,node,depth);
        node.depth=depth;
        nodes.push(node);
        if(deplist && (n=deplist.length)){
            var i = -1, n, c = node.deptreenodelist = [], v = 0, j = depth -1, d;
            
            while (++i < n) {
                d = previousrecurse(deplist[i], j, nodes);
                if(!isexistelem(node,d.children)){
    ...
    function childrenrecurse(node,depth,nodes){
      ...
      if (childs && (n = childs.length)) {
        ...
        while (++i < n) {
          d = childrenrecurse(childs[i], j, nodes);
          d.parent = node;
          c.push(d);
          v += d.value;
        }
    ...
    function recurse(node, depth, nodes) {
        previousrecurse(node,depth,nodes);
        nodes.shift();
        return childrenrecurse(node,depth,nodes);
    }
   ...
   function getmindepth(nodes){
          var min=nodes[0].depth;
          for(var i=0;i<nodes.length;i++){
              if(nodes[i].depth<min){
                  min=nodes[i].depth;
              }
          }
          return min;
   ...
   function getmindepthandshift(nodes){
       var min=getmindepth(nodes);
       if(min>=0){
           return;
       }else{
           var fixed=-min;
  ...
  function d3_layout_hierarchyDepTreeNodelist(d){
    return d.deptreenodelist;
  }
  ...
  d3.layout.tree = function() {
      function getmindepthnode(nodes){
          var min=nodes[0].depth,d=0;
    ...
    function tree(d, i) {
      var nodes = hierarchy.call(this, d, i), root = getmindepthnode(nodes);
